...
export function registerTeamTools(server) {
    server.tool('get_team_roster', 'Get the current roster for an MLB team.', {
        ...
        const data = await fetchMLB(`/api/v1/teams/${teamId}/roster`, {
        ...
        if (!roster.length)
            return { content: [{ type: 'text', text: 'No roster data found.' }] };
        ...
        const fmt = (players) => players.map((p) => {
        ...
        const sections = [];
        if (pitchers.length)
            sections.push(`**Pitchers (${pitchers.length})**\n${fmt(pitchers)}`);
        if (catchers.length)
            sections.push(`**Catchers (${catchers.length})**\n${fmt(catchers)}`);
        if (infielders.length)
            sections.push(`**Infielders (${infielders.length})**\n${fmt(infielders)}`);
        if (outfielders.length)
            sections.push(`**Outfielders (${outfielders.length})**\n${fmt(outfielders)}`);
        if (dh.length)
            sections.push(`**DH (${dh.length})**\n${fmt(dh)}`);
        if (other.length)
    ...
    server.tool('get_team_info', 'Get details about an MLB team including league, division, venue, and recent history.', {
        teamId: z.number().describe('MLB team ID.'),
    }, async ({ teamId }) => {
        const data = await fetchMLB(`/api/v1/teams/${teamId}`, {
        ...
        if (!team)
            return { content: [{ type: 'text', text: 'Team not found.' }] };
        const lines = [
            `## ${team.name}`,
