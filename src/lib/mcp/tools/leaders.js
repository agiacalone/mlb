...
export function registerLeadersTools(server) {
    server.tool('get_stat_leaders', 'Get MLB statistical leaders for any batting or pitching category.', {
        stat: z
            .string()
            .describe('Stat category. Examples: homeRuns, battingAverage, era, strikeouts, wins, stolenBases, rbi, ops, whip, saves, hits, runs, sluggingPercentage, onBasePercentage'),
        league: z
            .enum(['AL', 'NL', 'MLB'])
            .optional()
        ...
        const data = await fetchMLB('/api/v1/stats/leaders', {
