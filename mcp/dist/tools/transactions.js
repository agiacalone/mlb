...
export function registerTransactionTools(server) {
    server.tool('get_transactions', 'Get MLB roster transactions including trades, signings, DFA designations, IL placements, and call-ups.', {
        teamId: z
            .number()
            .optional()
        ...
        const data = await fetchMLB('/api/v1/transactions', {
            sportId: '1',
        ...
        if (type && type !== 'all') {
            ...
            transactions = transactions.filter((t) => keywords.some((k) => t.typeDesc?.toLowerCase().includes(k.toLowerCase()) ||
