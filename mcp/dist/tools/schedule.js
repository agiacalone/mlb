...
export function registerScheduleTools(server) {
    server.tool('get_schedule', 'Get the MLB schedule for a given date. Returns all games with scores, status, and matchups.', {
        date: z
            .string()
            .optional()
        ...
        const data = await fetchMLB('/api/v1/schedule', {
            sportId: '1',
            date: date ?? today(),
        ...
        if (!dates.length)
            return { content: [{ type: 'text', text: 'No games found for that date.' }] };
        const lines = [];
        for (const d of dates) {
            ...
            for (const game of d.games ?? []) {
                ...
                lines.push(`- **${awayName}${awayScore}** @ **${homeName}${homeScore}** — ${status}${flags ? ` [${flags}]` : ''} (gamePk: ${game.gamePk})`);
            }
        }
        return { content: [{ type: 'text', text: lines.join('\n') }] };
    });
    server.tool('get_game_recap', 'Get a detailed recap of a specific MLB game including scoring plays, box score summary, decisions, and game info.', {
        gamePk: z.number().describe('The MLB game ID (gamePk).'),
    }, async ({ gamePk }) => {
        const data = await fetchMLB(`/api/v1.1/game/${gamePk}/feed/live`, {
            fields: [
        ...
        if (decisions) {
            ...
            const getPlayer = (id) => id ? (players[`ID${id}`]?.fullName ?? `Player ${id}`) : null;
            if (decisions.winner)
                lines.push(`**W:** ${getPlayer(decisions.winner.id)}`);
            if (decisions.loser)
                lines.push(`**L:** ${getPlayer(decisions.loser.id)}`);
            if (decisions.save)
                lines.push(`**S:** ${getPlayer(decisions.save.id)}`);
        }
        // Game info
        if (info) {
            if (info.attendance)
                lines.push(`**Attendance:** ${info.attendance.toLocaleString()}`);
            if (info.gameDurationMinutes)
                lines.push(`**Duration:** ${Math.floor(info.gameDurationMinutes / 60)}h ${info.gameDurationMinutes % 60}m`);
        }
    ...
    server.tool('get_team_schedule', 'Get a team\'s upcoming or recent schedule for the current season.', {
        ...
        const dates = data?.dates ?? [];
        if (!dates.length)
            return { content: [{ type: 'text', text: 'No games found.' }] };
        const lines = [];
        for (const d of dates) {
            for (const game of d.games ?? []) {
