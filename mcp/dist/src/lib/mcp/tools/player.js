...
export function registerPlayerTools(server) {
    server.tool('search_player', 'Search for an MLB player by name. Returns a list of matching players with their IDs. Use the personId from results to call get_player_bio or get_player_stats.', {
        name: z.string().describe('Player name to search for (first name, last name, or both).'),
    }, async ({ name }) => {
        const data = await fetchMLB('/api/v1/people/search', {
            names: name,
            sportId: '1',
    ...
    server.tool('get_player_bio', 'Get biographical information for an MLB player.', {
        personId: z.number().describe('MLB player ID (personId). Use search_player to find this.'),
    }, async ({ personId }) => {
        const data = await fetchMLB(`/api/v1/people/${personId}`, {
            hydrate: 'currentTeam',
            fields: [
    ...
    server.tool('get_player_stats', 'Get hitting, pitching, or fielding statistics for an MLB player.', {
        ...
        const data = await fetchMLB(`/api/v1/people/${personId}/stats`, {
            stats: statType,
            group,
